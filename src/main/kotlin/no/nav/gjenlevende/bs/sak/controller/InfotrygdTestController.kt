...
class InfotrygdTestController(
    ...
    @Operation(
        ...
        description =
            "Henter alle vedtaksperioder for en gitt person basert på personIdent. " +
                "Returnerer barnetilsyn (GB) og skolepenger (GU) perioder separat. " +
                "Krever ALLE tre roller: SAKSBEHANDLER, BESLUTTER og VEILEDER.",
        security = [SecurityRequirement(name = "oauth2")],
    )
    fun hentPerioderForPerson(
        ...
        try {
            val response = infotrygdClient.hentPerioderForPersonSync(jwt.tokenValue, personIdent)
